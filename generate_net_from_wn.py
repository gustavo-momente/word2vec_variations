...
def create_parsers():
    ...
    parser = argparse.ArgumentParser(description='Generates a network starting model from vocabulary (exported by '
                                                 'word2vec), WordNet file data (generated with word_net_tree.py) and'
                                                 'its vector representation (generated by a pass of word2vec in '
                                                 'the reshaped corpus). \n For examples check ./Tests/wn_test/*.sh')
